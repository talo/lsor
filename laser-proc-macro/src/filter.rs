...
pub fn expand_derive_filter(input: TokenStream) -> TokenStream {
    ...
    match &ast.data {
        // Struct
        Data::Struct(data) => expand_derive_filter_for_struct(&ast.ident, &ast.attrs, data),
        Data::Enum(data) => expand_derive_filter_for_enum(&ast.ident, &ast.attrs, data),
...
fn expand_derive_filter_for_struct(
    ...
    if util::has_json_attr(attrs) {
        return expand_derive_json_filter_for_struct(ident, attrs, data);
    }
    ...
    let field_variants_impl = fields.named.iter().filter_map(|field| {
        ...
        if flat {
            ...
            Some(quote! { #filter_ident::#field_ident_camel_case(filter) => {
                driver.push(stringify!(#field_ident));
                filter.push_to_driver(driver);
            }})
        }
    });
